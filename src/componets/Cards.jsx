...
function Cards({ id, icon, name, description, isDarkMode, isActive, onToggle }) {
    return (
        <section className={`w-full p-5 md:p-6 rounded-2xl border shadow-xs flex flex-col justify-between min-h-44 md:min-h-47.5 transition-all duration-300 ${isDarkMode ? "bg-(--Neutral-800) border-(--Neutral-700)" : "bg-(--Neutral-0) border-(--Neutral-100)"}`}>
            <div className="upper-layer flex gap-4 items-start w-full">
                <div className="flex items-center justify-center shrink-0 w-10 h-10 md:w-12 md:h-12">
                    <img src={icon} alt={name} className="w-8 h-8 md:w-10 md:h-10 object-contain" />
                </div>
                <div className="texts flex-1">
                    <h3 className={`font-bold text-base md:text-lg mb-1 transition-colors ${isDarkMode ? "text-white" : "text-(--Neutral-900)"}`}>{name}</h3>
                    <p className={`text-xs md:text-sm leading-relaxed transition-colors ${isDarkMode ? "text-gray-400" : "text-(--Neutral-600)"}`}>{description}</p>
                </div>
            </div>
            <div className={`lower-layer mt-5 md:mt-6 pt-4 flex justify-between items-center border-t ${isDarkMode ? "border-(--Neutral-700)" : "border-(--Neutral-100)"}`}>
